	...
	var request_flag = false,
	   // 有提示的ajax请求标志,没返回之前不准发第二次请求
		...
		layer_html1 = '<div id="post_layer" style="width:100%;height:100%;background:#ffffff;opacity:0;margin:0;padding:0;position:fixed;left:0;top:0;z-index:999;"></div>'
					...
					+ '<p style="text-align:center;width:100%;font-size:0.32rem;padding-top:6%;padding-bottom:8%;">',
		layer_html2 = '</p></div>',
	...
	var tools = {
...

		{
		     ...
		     if(r!=null){
		     	...
		     	return  this.decryptByDES(decodeURI(unescape(r[2])),config.DESkey); 
            ...
            if(platform==1||platform==2){
	        	...
	        	if(registeropp){
				  ...
				  jq('.opomask').show();
	        	 	...
	        	 	window.location.href=config.gowechatloginpath+"?openId="+openid;
	        	...
	        	if(registeropp){
				  ...
				  jq('.opomask').show();
	        		...
	        		window.location.href=config.gowraploginpath;
		...
		askactivenum:function(actCodenum){
		    ...
		    var parameters=jq('input[name="uid"]').data();
			    ...
			    $.ajax({  
			        url : config.pageinteractionpath+"/act07/v1/01",  
			        type : "POST",  
			        dataType : "json",  
			        data: parameters,
...
//alert(res.code+"页面返回值")
			        	 if(res.code=="0000"){
                            ...
                            //alert(config.activenum+"===num")
                            // 开启第几个阶段提示信息做相应的改变
			        	    if(config.activenum==2){
			        	  	...
			        	  	//alert(res.code)
                               config.activenum=Number(config.activenum)+1;
                               // that.askactivenum(config.actCode2);
			        	  }
        ...
        collectlongstyle:function(){
        	   ...
        	   var parameters=jq('input[name="uid"]').data();
				   ...
				   parameters.device_token=this.GetQueryStringfnDES('device_token')||'';
				   parameters.actCode=config.actCode2;

               $.post(config.pageinteractionpath+"/act09/material/query",parameters,function(res){
                       if(res.code=="0000"){
                       	 jq.each(config.longlistarr,function(i,ele){
	...
	return {
...

		{
		     ...
		     if(r!=null){
		     	...
		     	//return  tools.decryptByDES(decodeURI(unescape(r[2])),config.DESkey); 
